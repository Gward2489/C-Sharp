...
namespace lists
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Dictionary<string, List<string>> travelLog = new Dictionary<string, List<string>>();

            foreach (string planet in planetList){
                List<string> visitedBy = new List<string>{};
                foreach (Dictionary<string, List<string>> craft in spacecraft){
                    foreach (KeyValuePair<string, List<string>> kvp in craft){
                        foreach(string planetChecked in kvp.Value){
                            if (planetChecked == planet) {
                                visitedBy.Add(kvp.Key);

                            }                            
                        }
                    }
                }
                travelLog.Add(planet, visitedBy);
            }

            foreach (KeyValuePair<string, List<string>> kvp in travelLog){
                string craftAndPlanets = $"{kvp.Key}: ";
                foreach (string craftName in kvp.Value){
                    craftAndPlanets += $"{craftName}, ";
                }
                char[] charsToTrim = {',',' '};
                Console.WriteLine(craftAndPlanets.TrimEnd(charsToTrim));
            }
        }
    }
}
